...
class Conta:
    ...
    def sacar(self, valor: float) -> bool:
        # Conta base: sem limite de saque por padrão, apenas saldo
        if valor <= 0:
            print("\n@@@ Operação falhou! O valor informado é inválido. @@@")
            return False
        if valor > self.saldo:
...
def criar_conta(agencia, numero_conta, usuarios, contas):
    ...
    if usuario:
        conta = ContaCorrente(agencia=agencia, numero=numero_conta, cliente=usuario)
...
def main():
    ...
    while True:
        ...
        if opcao == "d":
            try:
                numero = int(input("Informe o número da conta (C/C): "))
            except ValueError:
                print("\n@@@ Número de conta inválido. @@@")
                continue

            conta = buscar_conta_por_numero(numero, contas)
            if not conta:
            ...
            try:
                numero = int(input("Informe o número da conta (C/C) para exibir extrato: "))
            except ValueError:
                print("\n@@@ Número de conta inválido. @@@")
                continue
